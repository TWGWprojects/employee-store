...
if (!class_exists('OCWMA_front')) {
    ...
    class OCWMA_front {
        ...
        protected static $instance;
          ...
          ?>
              <div id="ocwma_billing_popup" class="ocwma_billing_popup_class woocommerce">
              </div>
          ...
          function ocwma_my_account_endpoint_content() {  
               ...
               $user = $wpdb->get_results( "SELECT * FROM {$tablename} WHERE type='billing' AND userid=".$user_id);
                ...
                if(!empty($user)){    
                  ...
                  echo "<tbody>";  
                    foreach($user as $row){    
                      ...
                      echo '<td >'.$user_data['billing_first_name'] .'&nbsp'.$user_data['billing_last_name'] ;
                      echo (!empty($user_data['billing_company']))? '<br>' .$user_data['billing_company'] :'' ;
                      echo '<br>' .$user_data['billing_address_1'];
                      echo (!empty($user_data['billing_address_2']))? '<br>' .$user_data['billing_address_2'] :'';
                      echo '<br>' .$user_data['billing_city'] .' ' .$user_data['billing_country'] ;
                      echo '<br>' .$user_data['billing_postcode'] .' '.$user_data['billing_state'] .'</td>';
                      echo '</td>';
                      echo '<td><button class="form_option_ship_edit" data-id="'.$user_id.'"  data-eid-ship="'.$row->id.'">edit</button></td><td><a href="?action=delete-ship&did-ship='.$row->id.'">Delete</a></td>';
                      echo '</tr>';  
                    }
                    echo "</tbody>";  
                      echo '</table>';
                ...
                if(!empty($user_shipping)){    
                    ...
                    foreach($user_shipping as $row){    
                      ...
                      echo '<tr>';
                      echo '<td >'.$user_data['reference_field'] .'</td>';
                      echo '<td >'.$user_data['shipping_first_name'] .'&nbsp'.$user_data['shipping_last_name'] ;
                      echo '<br>' .$user_data['shipping_company'] ;
                      echo '<br>' .$user_data['shipping_address_1'];
                      echo '<br>' .$user_data['shipping_address_2'] ;
                      echo '<br>' .$user_data['shipping_city'] .' ' .$user_data['shipping_country'] ;
                      echo '<br>' .$user_data['shipping_postcode'] .' '.$user_data['shipping_state'] .'</td>';
                      echo '</td>';
                      echo '<td><button class="form_option_ship_edit" data-id="'.$user_id.'"  data-eid-ship="'.$row->id.'">edit</button></td><td><a href="?action=delete-ship&did-ship='.$row->id.'">Delete</a></td>';
                      echo '</tr>';  
                      }   
            ...
            <div class="cus_menu">
                <div class="billling-button">
                  <button class="form_option_billing" data-id="<?php echo $user_id; ?>"><?php echo get_option( 'ocwma_head_title', 'Add Billing Address' );?></button>
                </div>
        ...
        function OCWMA_all_billing_address(){
         ...
         <select class="ocwma_select">
          ...
          <button class="form_option_billing button alt" data-id="<?php echo $user_id; ?>"><?php echo get_option( 'ocwma_head_title', 'Add Billing Address' );?></button>
